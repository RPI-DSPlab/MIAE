...
def load_and_create_predictions(attack: List[str], dataset: str, architecture: str, seeds: List[int] = None,
    ...
    target_dataset_path = f"/data/public/comp_mia_data/miae_experiment{aug_str}/target/{dataset}/"
    ...
    for att in attack:
        pred_list = []
        for s in seeds:
            pred_path = f"/data/public/comp_mia_data/miae_experiment{aug_str}/preds_sd{s}/{dataset}/{architecture}/{att}/pred_{att}.npy"
...
def plot_venn(pred_list: List[utils.Predictions], goal: str, graph_title: str, graph_path: str):
    """
    ...
    """
    if goal == "common_tp":
        utils.plot_venn_diagram(pred_list, graph_title, graph_path, goal)
...
if __name__ == '__main__':
    ...
    if args.graph_type == "venn":
        goal = "common_tp"
        # data processing
        pred_list = utils.data_process_for_venn(pred_dict, threshold=None, target_fpr=0.5)
        plot_venn(pred_list, goal, args.graph_title, args.graph_path)
