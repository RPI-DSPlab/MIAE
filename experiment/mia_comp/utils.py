    ...
    def adjust_fpr(self, target_fpr):
        ...
        if current_fpr < target_fpr:
            print(f"No adjustment is needed. " 
                    f"The current FPR is less than the target FPR.")
...
def plot_venn_diagram(pred_list: List[Predictions], title: str, save_path: str, goal: str = "different_attacks_seed", threshold: Optional[float] = None, target_fpr: Optional[float] = None):
    """
    ...
    """
    if len(pred_list) < 2 and goal == "common_TP":
