...
def data_process_for_venn(pred_dict: Dict[str, List[Predictions]], threshold: Optional[float] = None, target_fpr: Optional[float] = None) -> List[Predictions]:
    """
    ...
    if threshold is not None:
        ...
        for attack, pred_list in pred_dict.items():
            common_tp_pred = common_tp_preds(pred_list)
            pred_list.append(common_tp_pred)
        ...
        for attack, pred_obj_list in pred_dict.items():
            ...
            for pred in pred_obj_list:
                ...
                single_attack.append(adjusted_pred_obj)
...
def plot_venn_diagram(pred_list: List[Predictions], title: str, save_path: str, goal: str = "common_tp"):
    """
